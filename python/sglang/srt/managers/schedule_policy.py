...
class PrefillAdder:
    ...
    def _lock_node(self, last_node: TreeNode):
        # If last_node is None (e.g., for multi-vector embedding models that skip
        # prefix caching), there's nothing to lock, so just yield and return
    ...
    def add_one_req(
        ...
        with self._lock_node(req.last_node):
            ...
            if self.rem_chunk_tokens is None or input_tokens <= self.rem_chunk_tokens:
                # Non-chunked prefill
                self.can_run_list.append(req)
                # Skip locking if last_node is None (e.g., multi-vector embedding models)
